...
MONTHLY_CATEGORIES = {
    "Electricity (kWh)": {"esrs_tag": "ESRS-E1-M-01", "unit": "kWh", "co2_factor": 0.40, "scope": "Scope 2"},
    "Gas / Heating (kWh)": {"esrs_tag": "ESRS-E1-M-03", "unit": "kWh", "co2_factor": 0.20, "scope": "Scope 1"},
    "Water (m3)": {"esrs_tag": "ESRS-E3-1", "unit": "m3", "co2_factor": 1.052, "scope": "Scope 3"},
    "Diesel / Fuel (Liters)": {"esrs_tag": "ESRS-E1-S1-Fleet", "unit": "liters", "co2_factor": 2.68, "scope": "Scope 1"},
    "General Waste (kg)": {"esrs_tag": "ESRS-E5-5", "unit": "kg", "co2_factor": 0.50, "scope": "Scope 3"},
    "Recycling (kg)": {"esrs_tag": "ESRS-E5-5", "unit": "kg", "co2_factor": 0.02, "scope": "Scope 3"},
    "District Heating (kWh)": {"esrs_tag": "ESRS-E1-M-03", "unit": "kWh", "co2_factor": 0.28, "scope": "Scope 2"},
    "Other / Custom": {"esrs_tag": "ESRS-GENERIC", "unit": "units", "co2_factor": 0.0, "scope": "Scope 3"}
}
GRID_FACTORS = {
    "Austria": 0.13, "Germany": 0.38, "Switzerland": 0.03, "France": 0.06,
    "Italy": 0.23, "Spain": 0.19, "Netherlands": 0.37, "Belgium": 0.17,
    "Sweden": 0.02, "Denmark": 0.16, "Poland": 0.70, "Ireland": 0.33,
    "United Kingdom": 0.23, "Singapore": 0.41, "Philippines": 0.51,
    "USA": 0.39, "Other / Unknown": 0.40
}
MONTHS = ["January", "February", "March", "April", "May", "June",
          "July", "August", "September", "October", "November", "December"]
LOCATION_TYPES = ["Office / Headquarters", "Production Hall / Factory", "Warehouse / Logistics",
                  "Retail / Shop", "Data Center", "Construction Site", "Other"]

# --- CONFIGURATION ---
st.set_page_config(page_title="Ready for ESG", layout="wide", page_icon="logo.png")
...
st.markdown("""
    ...
    [data-testid="stAppViewContainer"] { background-color: #fdfef9; border: 20px solid #023425; }
    [data-testid="stHeader"] { background-color: transparent; }
    section[data-testid="stSidebar"] { background-color: #023425 !important; border-right: none; }
    section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] span,
    section[data-testid="stSidebar"] label, section[data-testid="stSidebar"] div,
    section[data-testid="stSidebar"] h1, section[data-testid="stSidebar"] h2,
    section[data-testid="stSidebar"] h3, section[data-testid="stSidebar"] small,
    section[data-testid="stSidebar"] .stMarkdown { color: #fdfef9 !important; }
    section[data-testid="stSidebar"] input { background-color: #416852 !important; border: 1px solid #8a9a93 !important; color: #fdfef9 !important; border-radius: 4px; }
    section[data-testid="stSidebar"] input::placeholder { color: #8a9a93 !important; }
    section[data-testid="stSidebar"] [data-baseweb="select"] > div { background-color: #416852 !important; border: 1px solid #8a9a93 !important; color: #fdfef9 !important; }
    section[data-testid="stSidebar"] div.stButton > button { background-color: transparent !important; border: 1px solid #8a9a93 !important; color: #fdfef9 !important; border-radius: 2px !important; }
    section[data-testid="stSidebar"] div.stButton > button:hover { background-color: #416852 !important; }
    section[data-testid="stSidebar"] [data-testid="stRadio"] label { color: #fdfef9 !important; }
    h1, h2, h3 { font-family: 'Cormorant Garamond', serif !important; font-weight: 500 !important; color: #023425 !important; letter-spacing: 0.02em !important; }
    div.stButton > button[kind="primary"] { background-color: #023425 !important; border: 1px solid #023425 !important; color: #fdfef9 !important; border-radius: 2px !important; font-family: 'DM Sans', sans-serif !important; font-weight: 500 !important; }
    div.stButton > button[kind="primary"]:hover { background-color: #416852 !important; border-color: #416852 !important; }
    div.stButton > button[kind="secondary"] { background-color: transparent !important; border: 1px solid #023425 !important; color: #023425 !important; border-radius: 2px !important; }
    div[data-testid="stMetricValue"] { color: #023425 !important; font-family: 'Cormorant Garamond', serif !important; font-size: 2rem !important; }
    section[data-testid="stSidebar"] hr { border-color: #416852 !important; }
    section[data-testid="stSidebar"] [data-testid="stImage"] img { border: 1px solid #fdfef9; border-radius: 50%; padding: 3px; max-width: 60px !important; width: 60px !important; }
    </style>
""", unsafe_allow_html=True)

# --- SESSION STATE INIT ---
if 'entry_stage' not in st.session_state: st.session_state['entry_stage'] = 'main'
...
def register(email, password, company):
    try:
        response = supabase.auth.sign_up({"email": email, "password": password, "options": {"data": {"company_name": company}}})
        st.session_state['user'] = response.user
        return True
    except Exception as e:
...
def setup_mfa():
    try:
        response = supabase.auth.mfa.enroll({"factor_type": "totp", "issuer": "Ready for ESG"})
...
def verify_mfa(code):
    try:
        ...
        verify = supabase.auth.mfa.verify({"factor_id": factor_id, "challenge_id": challenge.id, "code": code})
...
def add_exact_to_batch():
    ...
    if kwh > 0:
        ...
        factors = {"Green Electricity (Renewable)": 0.0, "Nuclear Power": 0.01, "Coal / Fossil Mix": 0.80}
        if source == "Grid Mix (National Average)":
            co2_factor = GRID_FACTORS.get(country, 0.40)
        else:
            co2_factor = factors.get(source, 0.40)
        co2 = kwh * co2_factor
        st.session_state['e1_batch_list'].append({"type": "Exact", "source": source, "liters": kwh, "notes": f"{notes} | Country: {country} | Factor: {co2_factor} kg/kWh", "co2": co2})
...
def add_estimate_to_batch():
    ...
    st.session_state['e1_batch_list'].append({"type": "Estimate", "source": f"ESTIMATE: {use} ({age})", "liters": est_kwh, "notes": f"{n_b}x {area}m2 | Country: {country} | Factor: {co2_factor} kg/kWh", "co2": est_co2})
...
def upload_batch():
    ...
    if current_company and st.session_state['e1_batch_list']:
        ...
        for item in st.session_state['e1_batch_list']:
            ...
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": item['type'], "esrs_tag": meta['tag'], "description": item.get('notes', ''), "methodology": f"Calculated via Boku-Factor ({meta['unit']})", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
...
def remove_last_s2():
    if 's2_batch_list' in st.session_state and st.session_state['s2_batch_list']: st.session_state['s2_batch_list'].pop()

def remove_last_s3():
    if 's3_batch_list' in st.session_state and st.session_state['s3_batch_list']: st.session_state['s3_batch_list'].pop()

def remove_last_s4():
    if 's4_batch_list' in st.session_state and st.session_state['s4_batch_list']: st.session_state['s4_batch_list'].pop()
...
def upload_heating_batch():
    ...
    if current_company and st.session_state['e1_2_batch_list']:
        ...
        for item in st.session_state['e1_2_batch_list']:
            ...
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": item['type'], "esrs_tag": meta['tag'], "description": item.get('notes', ''), "methodology": f"Boku-Standard-Factor ({meta['unit']})", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            try:
                supabase.table("esg_data_entries").insert(entries).execute()
                st.toast(f"Uploaded {len(entries)} records successfully.")
...
def add_distance_estimate_to_batch():
    ...
    if km > 0:
        ...
        st.session_state.widget_m_km_2 = 0.0
    else:
        st.toast("Distance must be greater than 0.")
...
def upload_mobility_batch():
    ...
    if current_company and st.session_state['e2_batch_list']:
        ...
        for item in st.session_state['e2_batch_list']:
            ...
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": item['type'], "esrs_tag": meta['tag'], "description": item.get('notes', ''), "methodology": f"Boku-Calculation ({meta['unit']})", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
            st.toast(f"Uploaded {len(entries)} records successfully.")
...
def add_travel_to_batch():
    ...
    if amount > 0:
        ...
        if "Flight" in travel_type:
            unit_label = "Flights"
            flight_factors = {"Flight: Short Haul (< 500 km)": 75.0, "Flight: Medium Haul (500-1500 km)": 200.0, "Flight: Long Haul (1500-3700 km)": 450.0, "Flight: Intercontinental (3700-5000 km)": 645.0, "Flight: Ultra Long Haul (> 5000 km)": 1200.0}
            ...
            unit_label = "km"
            km_factors = {"Train (National/Regional)": 0.06, "Train (Long Distance/High Speed)": 0.01, "Private Car (Reimbursement)": 0.18}
            co2 = amount * km_factors.get(travel_type, 0.1)
        final_notes = f"{notes} ({unit_label})"
        if file_url:
            final_notes += f" | Evidence: {file_url}"
        st.session_state['e3_batch_list'].append({"type": "Travel", "source": travel_type, "liters": amount, "notes": final_notes, "co2": co2})
...
def upload_travel_batch():
    ...
    if current_company and st.session_state['e3_batch_list']:
        ...
        for item in st.session_state['e3_batch_list']:
            ...
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": "Travel Record", "esrs_tag": meta['tag'], "description": item.get('notes', ''), "methodology": f"Boku-Travel-Factor ({meta['unit']})", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase is not None:
            try:
                supabase.table("esg_data_entries").insert(entries).execute()
                st.toast(f"Uploaded {len(entries)} records successfully.")
...
def add_commute_to_batch():
    ...
    if employees > 0:
        distance_map = {"Short (< 10 km)": 5.0, "Medium (10 - 30 km)": 20.0, "Long (30 - 60 km)": 45.0, "Very Long (> 60 km)": 80.0}
        km_one_way = distance_map.get(dist_cat, 10.0)
        factors = {"Car (Petrol/Diesel)": 0.18, "Car (Electric)": 0.0, "Public Transport (Bus/Train)": 0.04, "Motorbike": 0.10, "Bicycle / Walk": 0.0}
        annual_km = km_one_way * 2 * days_week * 46 * employees
        co2 = annual_km * factors.get(mode, 0.15)
        note = f"{employees} Pers. ({dist_cat})"
        st.session_state['e4_batch_list'].append({"type": "Commuting", "source": f"Commute: {mode}", "liters": annual_km, "notes": note, "co2": co2})
...
def upload_commute_batch():
    ...
    if current_company and st.session_state['e4_batch_list']:
        ...
        for item in st.session_state['e4_batch_list']:
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": "Commuting Group", "esrs_tag": "ESRS-E1-S3-Commuting", "description": item.get('notes', ''), "methodology": "Boku-Employee-Commuting-Model", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
            st.toast(f"Uploaded {len(entries)} records successfully.")
...
def add_waste_volume_to_batch():
    ...
    if quantity > 0:
        ...
        density_map = {"General Waste (Residual)": 0.15, "Paper / Cardboard": 0.06, "Plastic / Packaging": 0.05, "Glass": 0.35, "Organic / Bio": 0.40, "Electronic Waste": 0.25}
        est_weight_kg = total_liters * density_map.get(waste_type, 0.15)
        co2_factors = {"General Waste (Residual)": 0.5, "Paper / Cardboard": 0.02, "Plastic / Packaging": 0.05, "Glass": 0.02, "Organic / Bio": 0.1, "Electronic Waste": 0.05}
        co2 = est_weight_kg * co2_factors.get(waste_type, 0.5)
        note = f"{quantity}x {container_size}L ({frequency}) | {provider}"
        st.session_state['e5_batch_list'].append({"type": "Waste", "source": f"Waste: {waste_type}", "liters": est_weight_kg, "notes": note, "co2": co2})
        st.toast("Record added to list.")
...
def upload_waste_batch():
    ...
    if current_company and st.session_state['e5_batch_list']:
        ...
        for item in st.session_state['e5_batch_list']:
            ...
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": item['liters'], "co2_kg": item['co2'], "type": "Waste Record", "esrs_tag": meta['tag'], "description": item.get('notes', ''), "methodology": "Boku-Waste-Factor", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
            st.toast(f"Uploaded {len(entries)} records successfully.")
...
def add_water_to_batch():
    ...
    if m3 > 0:
        ...
        co2 = m3 * co2_factor
        st.session_state['e_water_batch_list'].append({"type": "Water", "source": f"Water: {source}", "liters": m3, "notes": f"{m3} m3 consumed", "co2": co2})
        st.toast("Record added to list.")
...
def add_water_estimate_to_batch():
    ...
    if occupants > 0:
        ...
        st.session_state['e_water_batch_list'].append({"type": "Water", "source": "Mains Water (Estimated)", "liters": m3_calc, "notes": note, "co2": co2})
        st.toast("Record added to list.")
...
def add_bio_check_to_batch():
    ...
    st.session_state['e_bio_batch_list'].append({"type": "Pollution", "source": "ESRS E2: Pollution Check", "liters": 0, "notes": f"Hazardous Substances: {pollutants}", "co2": 0})
    st.session_state['e_bio_batch_list'].append({"type": "Biodiversity", "source": "ESRS E4: Biodiversity", "liters": 0, "notes": f"Site Location: {area_type}", "co2": 0})
    st.toast("Record added to list.")
...
def save_policies_to_batch():
    ...
    st.session_state['e6_batch_list'].append({"type": "Policy", "source": "Governance: Environmental Policy", "liters": 0, "notes": f"Status: {has_policy}", "co2": 0})
    ...
    if target_type != "No defined targets":
        st.session_state['e6_batch_list'].append({"type": "Policy", "source": "Strategy: Emission Targets", "liters": 0, "notes": target_note, "co2": 0})
    circular_strat = st.session_state.get('widget_p_circular', "")
    if circular_strat:
        st.session_state['e6_batch_list'].append({"type": "Policy", "source": "Circular Economy: Strategy", "liters": 0, "notes": circular_strat, "co2": 0})
    st.toast("Record added to list.")
...
def upload_policies_batch():
    ...
    if current_company and st.session_state['e6_batch_list']:
        ...
        for item in st.session_state['e6_batch_list']:
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": 0, "co2_kg": 0, "type": "Policy/Strategy", "esrs_tag": "ESRS-E1-2", "description": item['notes'], "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            try:
                supabase.table("esg_data_entries").insert(entries).execute()
                st.toast(f"Uploaded {len(entries)} records successfully.")
...
def save_s1_basic_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "S1: Workforce Data", "value_raw": hc, "co2_kg": 0, "type": "Exact", "esrs_tag": "ESRS-S1-6", "description": note, "methodology": "HR Record Export", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
...
def save_s1_health_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "S1.2: Health & Safety", "value_raw": accidents, "co2_kg": 0, "type": "Social", "esrs_tag": "ESRS-S1-14", "description": note, "methodology": "Accident Log / Insurance Data", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        supabase.table("esg_data_entries").insert([entry]).execute()
...
def save_s1_wages_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "S1.3: Wages & Gender Pay Gap", "value_raw": gap_pct, "co2_kg": 0, "type": "Calculated", "esrs_tag": "ESRS-S1-16", "description": note, "methodology": "Payroll Analysis", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        if supabase is not None:
            supabase.table("esg_data_entries").insert([entry]).execute()
...
def save_s1_training_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "S1.4: Training & Skills", "value_raw": total_h, "co2_kg": 0, "type": "Social", "esrs_tag": "ESRS-S1-13", "description": note, "methodology": "Training Management System", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        supabase.table("esg_data_entries").insert([entry]).execute()
...
def add_s2_to_batch():
    ...
    st.session_state['s2_batch_list'].append({"type": "Qualitative", "source": "S2: Workers in the Value Chain", "liters": 0, "notes": note, "co2": 0})
...
def upload_s2_batch():
    ...
    if current_company and st.session_state['s2_batch_list']:
        ...
        for item in st.session_state['s2_batch_list']:
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": 0, "co2_kg": 0, "type": "Social", "esrs_tag": "ESRS-S2", "description": item['notes'], "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
...
def add_s3_to_batch():
    ...
    st.session_state['s3_batch_list'].append({"type": "Qualitative", "source": "S3: Affected Communities", "liters": 0, "notes": note, "co2": 0})
...
def upload_s3_batch():
    ...
    if current_company and st.session_state['s3_batch_list']:
        ...
        for item in st.session_state['s3_batch_list']:
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": 0, "co2_kg": 0, "type": "Social", "esrs_tag": "ESRS-S3", "description": item['notes'], "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
            st.toast("Uploaded records successfully.")
...
def add_s4_to_batch():
    ...
    st.session_state['s4_batch_list'].append({"type": "Qualitative", "source": "S4: Consumers & End-users", "liters": 0, "notes": note, "co2": 0})
...
def upload_s4_batch():
    ...
    if current_company and st.session_state['s4_batch_list']:
        ...
        for item in st.session_state['s4_batch_list']:
            entries.append({"company": current_company, "fuel_type": item['source'], "value_raw": 0, "co2_kg": 0, "type": "Social", "esrs_tag": "ESRS-S4", "description": item['notes'], "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"})
        if supabase:
            supabase.table("esg_data_entries").insert(entries).execute()
            st.toast("Uploaded records successfully.")
...
def save_g1_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "G1: Business Conduct", "value_raw": 0, "co2_kg": 0, "type": "Governance", "esrs_tag": "ESRS-G1", "description": note, "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        if supabase:
            supabase.table("esg_data_entries").insert([entry]).execute()
...
def save_g2_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "G2: Management & Strategy", "value_raw": 0, "co2_kg": 0, "type": "Governance", "esrs_tag": "ESRS-G2", "description": note, "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        if supabase:
            supabase.table("esg_data_entries").insert([entry]).execute()
...
def save_g3_direct():
    ...
    current_company = st.session_state.get('current_company_id', '')
    if current_company:
        entry = {"company": current_company, "fuel_type": "G3: Supplier Relations", "value_raw": 0, "co2_kg": 0, "type": "Governance", "esrs_tag": "ESRS-G1-6", "description": note, "methodology": "Management Disclosure", "date_of_service": f"{st.session_state.get('current_year', '2024')}-12-31"}
        if supabase:
            supabase.table("esg_data_entries").insert([entry]).execute()
...
if 'user' not in st.session_state:
    ...
    with tab_register:
        with st.form("register_form"):
            st.markdown(f"**{t('create_account', lang)}**")
            reg_email = st.text_input(t("email", lang))
            reg_password = st.text_input(t("password", lang), type="password")
            reg_password2 = st.text_input(t("confirm_password", lang), type="password")
            reg_company = st.text_input(t("company_name", lang), placeholder="e.g. Muster GmbH")
            submitted = st.form_submit_button(t("create_account", lang), type="primary", use_container_width=True)
            if submitted:
        ...
        for sub in subs:
            if sub.get("status") == "active":
                ...
                if expires:
                    ...
                    if expires_dt > datetime.now(timezone.utc): return sub
                else:
                    return sub
        return None
    except Exception as e:
    ...
    with col2:
        with st.container(border=True):
            ...
            with st.form("request_access_form"):
                ...
                req_plan = st.selectbox(t("desired_package", lang), ["ESG Ready (1.099 EUR)", "ESG Certified (1.799 EUR)", "ESG Partner (3.299 EUR)", "Erstgespräch (kostenlos)"])
                ...
                if submitted:
                    if not req_name or not req_company:
                        ...
                        try:
                            ...
                            supabase.table("subscriptions").insert({"user_id": user_id, "email": user_email, "status": "pending", "plan": req_plan}).execute()
...
with st.sidebar:
    st.markdown("""
        ...
        [data-testid="stSidebar"] [data-testid="stImage"] { display: flex; justify-content: flex-start !important; margin-top: -5px !important; margin-left: 0px !important; margin-bottom: 10px !important; }
        </style>
    """, unsafe_allow_html=True)
    ...
    with st.container(border=True):
        if 'current_company_id' not in st.session_state or st.session_state['current_company_id'] == '':
            ...
            default_company = (user_meta.get('company_name') or user_meta.get('raw_user_meta_data', {}).get('company_name', ''))
                ...
                if supabase:
                    try:
                        ...
                        if profiles:
                            ...
                            st.session_state['company_setup_complete'] = True if current_year_profile else False
                            st.session_state['setup_needs_review'] = False if current_year_profile else True
                        else:
    ...
    if menu == t("data_entry", lang):
        st.markdown("---")
        if st.button(t("reset_wizard", lang)):
            st.session_state['entry_stage'] = 'main'
            for key in ['e1_batch_list', 'e1_2_batch_list', 'e2_batch_list', 'e3_batch_list', 'e4_batch_list', 'e5_batch_list', 'e6_batch_list', 'e_water_batch_list', 'e_bio_batch_list', 's1_batch_list', 's1_2_batch_list', 's1_3_batch_list', 's1_4_batch_list', 's2_batch_list', 's3_batch_list', 's4_batch_list']:
                st.session_state[key] = []
            for key in ['e1_step_complete', 'e1_2_step_complete', 'e2_step_complete', 'e3_step_complete', 'e4_step_complete', 'e5_step_complete', 'e6_step_complete', 'e_water_step_complete', 'e_bio_step_complete', 's1_step_complete', 's1_2_step_complete', 's1_3_step_complete', 's1_4_step_complete', 's2_step_complete', 's3_step_complete', 's4_step_complete']:
...
if not company_name:
    ...
    with c2:
        st.container(border=True).markdown(f"**{t('eu_compliance', lang)}** {t('eu_compliance_text', lang)}", unsafe_allow_html=True)
    st.info(t("enter_org_name", lang))
    st.stop()

if menu == t("dashboard", lang):
    st.subheader(f"{t('executive_scorecard', lang)}: {company_name}")
    ...
    if rows:
        ...
        st.markdown(f"### {t('overall_performance', lang)}")
        c1, c2, c3, c4 = st.columns(4)
        c1.metric(t("total_carbon", lang), f"{total_co2_t:,.1f} t CO2e")
        c2.metric(t("total_records", lang), len(df))
        score_color = "normal" if quality_score >= 80 else "inverse"
        c3.metric(t("data_quality", lang), f"{quality_score} / 100", t("high_precision", lang) if quality_score >= 80 else t("needs_exact_data", lang), delta_color=score_color)
        esrs_tags_used = len(df['esrs_tag'].unique())
        c4.metric(t("esrs_modules", lang), f"{esrs_tags_used} Tags")
        st.markdown("---")

        st.markdown(f"### {t('carbon_risk', lang)}")
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            with st.container(border=True):
                st.markdown(f"**{t('data_readiness', lang)}**")
        ...
        st.markdown(f"### {t('sustainability_targets', lang)}")
        with st.container(border=True):
            targets = df[df['fuel_type'] == 'Strategy: Emission Targets']
            if not targets.empty:
                target_desc = targets.iloc[0]['description']
                st.markdown(f"**{t('current_target', lang)}** {target_desc}")
                st.progress(15, text=t("target_progress", lang))
            else:
                st.info(t("no_targets", lang))
        st.markdown("---")
        ...
        with st.expander(t("view_manage_data", lang)):
            ...
            st.markdown(f"#### {t('delete_record', lang)}")
            if 'id' in df.columns:
                c_del1, c_del2 = st.columns([3, 1])
                with c_del1:
                    record_options = {f"ID {row['id']} - {row['fuel_type']} (Value: {row['value_raw']})": row['id'] for index, row in df.iterrows()}
                    selected_record_display = st.selectbox(t("select_record_delete", lang), options=list(record_options.keys()))
                with c_del2:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button(t("delete_button", lang), type="primary", use_container_width=True):
                        ...
                        try:
                            ...
                            st.success(t("record_deleted", lang))
                            time.sleep(1)
                            st.rerun()
                        except Exception as e:
                            st.error(f"{t('delete_failed', lang)} {e}")
            else:
    ...
    if st.session_state['entry_stage'] == 'review_setup':
        ...
        with st.container(border=True):
            ...
            if c1.button(t("use_existing_profile", lang), type="primary", use_container_width=True):
                ...
                if supabase: supabase.table("esg_data_entries").insert([entry]).execute()
        ...
        with st.container(border=True):
            ...
            elif ind == "Logistics / Transport": defaults.extend(["E2: Pollution", "S2: Value Chain Workers", "S3: Affected Communities"])
            elif ind == "Agriculture / Food": defaults.extend(["E2: Pollution", "E3: Water", "E4: Biodiversity", "S2: Value Chain Workers"])
            elif ind == "Retail / Wholesale": defaults.extend(["E5: Waste & Circular Economy", "S2: Value Chain Workers", "S4: Consumers"])
            elif ind == "IT / Software": defaults.extend(["S4: Consumers"])
            ...
            if st.button(t("save_profile_btn", lang), type="primary", use_container_width=True):
                if len(desc) < 20:
                    ...
                    st.session_state['setup_needs_review'] = False
                    import json
                    prof = {"industry": ind, "size": size, "footprint": footprint, "hq_country": hq_country, "other_countries": other_countries, "description": desc, "material_topics": mat_topics}
                    entry = {"company": st.session_state.get('current_company_id'), "fuel_type": "Profile Data", "value_raw": 0, "co2_kg": 0, "type": "Company Setup", "esrs_tag": "SETUP", "description": json.dumps(prof), "methodology": "System", "date_of_service": f"{st.session_state.get('current_year')}-01-01"}
                    if supabase:
        ...
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'main'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'env_intro'; st.rerun()
        ...
        st.markdown(f"Add all your buildings/meters to the list below. **{t('review_data', lang)} and upload in batch.**")
        tab_exact, tab_estimate = st.tabs([t("exact_records", lang), t("estimation_calculator", lang)])
        with tab_exact:
            with st.container(border=True):
                st.markdown(f"**{t('add_record', lang)}**")
                c1, c2 = st.columns(2)
                with c1:
                ...
                st.selectbox("Country / Grid Zone", options=list(GRID_FACTORS.keys()), key="widget_e1_country")
                if 'file_key_e1' not in st.session_state: st.session_state['file_key_e1'] = str(uuid.uuid4())
                st.file_uploader(t("upload_invoice", lang), type=["pdf", "jpg", "png", "jpeg"], key=st.session_state['file_key_e1'])
                if 'widget_notes_exact' not in st.session_state: st.session_state['widget_notes_exact'] = ""
                st.text_input(t("reference_note", lang), placeholder="e.g. Meter A", key="widget_notes_exact")
                st.button(t("add_to_list", lang), on_click=add_exact_to_batch, key="btn_add_e1", type="primary")
        with tab_estimate:
            st.warning(t("only_if_unavailable", lang))
            with st.container(border=True):
                st.markdown(f"**{t('add_estimate', lang)}**")
                c1, c2 = st.columns(2)
                with c1:
                ...
                st.selectbox("Country / Grid Zone", options=list(GRID_FACTORS.keys()), key="widget_e1_country_est")
        ...
        if st.session_state['e1_batch_list']:
            st.subheader(t("review_data", lang))
            df_batch = pd.DataFrame(st.session_state['e1_batch_list'])
            st.dataframe(df_batch[["source", "liters", "co2"]], use_container_width=True)
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_e1, key="rm_e1")
            c2.button(t("confirm_upload", lang), type="primary", on_click=upload_batch, key="up_e1")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e1"): st.session_state['entry_stage'] = 'e1_intro'; st.rerun()
        if st.session_state['e1_step_complete']:
            if c2.button("Next: Module E1.2 (Heating)", type="primary", key="next_e1"): st.session_state['entry_stage'] = 'e1_2_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e1"): st.session_state['entry_stage'] = 'e1_2_intro'; st.rerun()
        ...
        **Purchased Thermal Energy (Scope 2).**
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e1_form'; st.rerun()
        ...
        with tab_exact:
            with st.container(border=True):
                ...
                if 'file_key_heat_exact' not in st.session_state: st.session_state['file_key_heat_exact'] = str(uuid.uuid4())
                ...
                st.text_input(t("reference_note", lang), placeholder="e.g. Headquarters Heating", key="widget_h_notes")
                st.button(t("add_to_list", lang), on_click=add_heating_exact_to_batch, key="btn_add_e12", type="primary")
        with tab_estimate:
            st.warning(t("only_if_unavailable", lang))
            with st.container(border=True):
                st.markdown(f"**{t('add_estimate', lang)}**")
        ...
        if st.session_state['e1_2_batch_list']:
            ...
            st.dataframe(df_batch[["source", "liters", "co2"]], use_container_width=True)
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_heating, key="rm_e12")
            c2.button(t("confirm_upload", lang), type="primary", on_click=upload_heating_batch, key="up_e12")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e12"): st.session_state['entry_stage'] = 'e1_2_intro'; st.rerun()
        if st.session_state['e1_2_step_complete']:
            if c2.button("Next: Module E2 (Scope 1)", type="primary", key="next_e12"): st.session_state['entry_stage'] = 'e2_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e12"): st.session_state['entry_stage'] = 'e2_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e1_2_form'; st.rerun()
        ...
        with tab_exact:
            with st.container(border=True):
                ...
                if 'file_key_m_exact' not in st.session_state: st.session_state['file_key_m_exact'] = str(uuid.uuid4())
                ...
                st.button(t("add_to_list", lang), on_click=add_fuel_exact_to_batch, key="btn_add_e2", type="primary")
        with tab_estimate:
            st.warning(t("only_if_unavailable", lang))
            with st.container(border=True):
                st.markdown(f"**{t('add_estimate', lang)}**")
        ...
        if st.session_state['e2_batch_list']:
            ...
            st.dataframe(df_batch[["source", "liters", "co2"]], use_container_width=True)
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_e2, key="rm_e2")
            c2.button(t("confirm_upload", lang), type="primary", on_click=upload_mobility_batch, key="up_e2")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e2"): st.session_state['entry_stage'] = 'e2_intro'; st.rerun()
        if st.session_state['e2_step_complete']:
            if c2.button("Next: Module E3 (Business Travel)", type="primary", key="next_e2"): st.session_state['entry_stage'] = 'e3_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e2"): st.session_state['entry_stage'] = 'e3_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e2_form'; st.rerun()
        ...
        with st.container(border=True):
            st.markdown(f"**{t('add_to_list', lang)}**")
            c1, c2 = st.columns(2)
            with c1:
                if 'widget_t_type' not in st.session_state: st.session_state['widget_t_type'] = "Flight: Short Haul (< 500 km)"
                travel_options = ["Flight: Short Haul (< 500 km)", "Flight: Medium Haul (500-1500 km)", "Flight: Long Haul (1500-3700 km)", "Flight: Intercontinental (3700-5000 km)", "Flight: Ultra Long Haul (> 5000 km)", "Train (National/Regional)", "Train (Long Distance/High Speed)", "Hotel Stay (per Night)", "Private Car (Reimbursement)"]
            ...
            if 'file_key_travel' not in st.session_state: st.session_state['file_key_travel'] = str(uuid.uuid4())
            ...
            st.button(t("add_to_list", lang), on_click=add_travel_to_batch, key="btn_add_e3", type="primary")
        if st.session_state['e3_batch_list']:
            ...
            c_rm.button(t("remove_last", lang), on_click=remove_last_e3, key="rm_e3_btn")
            c_up.button(t("confirm_upload", lang), type="primary", on_click=upload_travel_batch, key="up_e3_btn")
        st.markdown("---")
        c_back, c_next = st.columns([1, 4])
        if c_back.button(t("back", lang), key="nav_back_e3"): st.session_state['entry_stage'] = 'e3_intro'; st.rerun()
        if st.session_state.get('e3_step_complete', False):
            if c_next.button("Next: Module E4 (Commuting)", type="primary", key="nav_next_e3"): st.session_state['entry_stage'] = 'e4_intro'; st.rerun()
        else:
            if c_next.button(t("skip", lang), key="nav_skip_e3"): st.session_state['entry_stage'] = 'e4_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e3_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.selectbox("Transport Mode", ["Car (Petrol/Diesel)", "Car (Electric)", "Public Transport (Bus/Train)", "Motorbike", "Bicycle / Walk"], key="widget_c_mode")
                st.selectbox("Commute Distance (One-Way)", ["Short (< 10 km)", "Medium (10 - 30 km)", "Long (30 - 60 km)", "Very Long (> 60 km)"], key="widget_c_dist_cat")
            with c2:
            ...
            st.button(t("add_to_list", lang), on_click=add_commute_to_batch, key="btn_add_e4", type="primary")
        st.markdown("---")
        if st.session_state['e4_batch_list']:
            ...
            st.dataframe(df_batch[["source", "liters", "notes", "co2"]], use_container_width=True)
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_e4, key="rm_e4")
            c2.button(t("confirm_upload", lang), type="primary", on_click=upload_commute_batch, key="up_e4")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e4"): st.session_state['entry_stage'] = 'e4_intro'; st.rerun()
        if st.session_state['e4_step_complete']:
            if c2.button("Next: Module E5 (Waste)", type="primary", key="next_e4"): st.session_state['entry_stage'] = 'e5_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e4"): st.session_state['entry_stage'] = 'e5_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e4_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.selectbox("Waste Type", ["General Waste (Residual)", "Paper / Cardboard", "Plastic / Packaging", "Glass", "Organic / Bio", "Electronic Waste"], key="widget_w_type")
                st.selectbox("Collection Frequency", ["Weekly", "Monthly", "One-time (Yearly)"], key="widget_w_freq")
                st.selectbox("Service Provider", ["Municipal / City Service", "Private Contractor", "Recycling Center (Self-drop)"], key="widget_w_provider")
            with c2:
            ...
            st.button(t("add_to_list", lang), on_click=add_waste_volume_to_batch, key="btn_add_e5", type="primary")
        st.markdown("---")
        if st.session_state['e5_batch_list']:
            ...
            st.dataframe(df_batch[["source", "liters", "notes", "co2"]], use_container_width=True)
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_e5, key="rm_e5")
            c2.button(t("confirm_upload", lang), type="primary", on_click=upload_waste_batch, key="up_e5")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e5"): st.session_state['entry_stage'] = 'e5_intro'; st.rerun()
        if st.session_state['e5_step_complete']:
            if c2.button("Next: Module E3 (Water)", type="primary", key="next_e5"): st.session_state['entry_stage'] = 'water_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e5"): st.session_state['entry_stage'] = 'water_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'e5_form'; st.rerun()
        ...
        tab_exact, tab_est = st.tabs(["Exact Meter (m³)", t("estimation_calculator", lang)])
        with tab_exact:
            with st.container(border=True):
                st.markdown(f"**{t('add_record', lang)}**")
                c1, c2 = st.columns(2)
                ...
                if 'file_key_e3' not in st.session_state: st.session_state['file_key_e3'] = str(uuid.uuid4())
                ...
                st.text_input(t("reference_note", lang), placeholder="e.g. Main Building", key="widget_notes_water")
                st.button(t("add_to_list", lang), on_click=add_water_to_batch, key="btn_add_water", type="primary")
        with tab_est:
            st.warning(t("only_if_unavailable", lang))
            with st.container(border=True):
                st.markdown(f"**{t('add_estimate', lang)}**")
        ...
        if st.session_state['e_water_batch_list']:
            ...
            c_rm, c_up = st.columns(2)
            c_rm.button(t("remove_last", lang), on_click=remove_last_water, key="rm_wat")
            if c_up.button(t("confirm_upload", lang), type="primary", use_container_width=True):
                try:
                    company = st.session_state.get('current_company_id', 'Unknown')
                    year = st.session_state.get('current_year', '2024')
                    records_to_insert = []
                    for item in st.session_state['e_water_batch_list']:
                        records_to_insert.append({"company": company, "date_of_service": f"{year}-12-31", "esrs_tag": "ESRS-E3-1", "type": "Water Consumption", "fuel_type": item["source"], "value_raw": item["liters"], "co2_kg": item["co2"], "description": item["notes"]})
                    supabase.table("esg_data_entries").insert(records_to_insert).execute()
                    st.session_state['e_water_batch_list'] = []
                    st.session_state['e_water_step_complete'] = True
                    st.success(t("saved_successfully", lang))
                    time.sleep(1)
                    st.rerun()
                except Exception as e:
        ...
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_wat"): st.session_state['entry_stage'] = 'water_intro'; st.rerun()
        if st.session_state.get('e_water_step_complete', False):
            if c2.button("Next: Modules E2 & E4 (Pollution & Bio)", type="primary", key="next_wat"): st.session_state['entry_stage'] = 'bio_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_wat"): st.session_state['entry_stage'] = 'bio_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'water_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            st.selectbox("Do you handle or emit hazardous substances (Microplastics, SVHC)?", ["None (Office/Service standard)", "Yes, managed internally", "Yes, high impact"], key="widget_bio_poll")
            st.markdown("---")
            st.markdown("**2. ESRS E4: Biodiversity**")
            st.selectbox("Is your operational site located in or near a biodiversity sensitive area?", ["Urban / Industrial Zone (No Impact)", "Near Protected Area (Natura 2000)", "In Protected Area"], key="widget_bio_area")
            st.button(t("add_to_list", lang), on_click=add_bio_check_to_batch, type="primary")
        st.markdown("---")
        if st.session_state['e_bio_step_complete']:
            st.success(t("saved_successfully", lang))
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_bio"): st.session_state['entry_stage'] = 'bio_intro'; st.rerun()
        if st.session_state['e_bio_step_complete'] or len(st.session_state['e_bio_batch_list']) > 0:
            if c2.button("Next: Module E6 (Strategy)", type="primary", key="next_bio"): st.session_state['entry_stage'] = 'e6_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_bio"): st.session_state['entry_stage'] = 'e6_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'bio_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            st.selectbox("Do you have a formal Environmental Policy signed by management?", ["No", "Yes, internal only", "Yes, published publicly"], key="widget_p_policy")
            st.markdown("---")
            st.markdown("**2. Ambition & Targets**")
            target_selection = st.selectbox("What are your Emission Reduction Targets?", ["No defined targets", "Reduce by X% (Short/Mid Term)", "Net Zero (Long Term / SBTi)"], key="widget_p_targets")
            if "Reduce" in target_selection:
                st.info("Please specify your reduction goal:")
                c1, c2 = st.columns(2)
                with c1: st.number_input("Reduction Target (%)", min_value=1, max_value=100, value=20, step=1, key="widget_p_target_pct")
                with c2: st.number_input("Target Year", min_value=2024, max_value=2060, value=2030, step=1, key="widget_p_target_year")
            ...
            st.button(t("add_to_list", lang), on_click=save_policies_to_batch, key="btn_add_e6", type="primary")
        st.markdown("---")
        if st.session_state['e6_batch_list']:
            ...
            st.button(t("confirm_upload", lang), on_click=upload_policies_batch, type="primary", use_container_width=True, key="up_e6")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_e6"): st.session_state['entry_stage'] = 'e6_intro'; st.rerun()
        if st.session_state.get('e6_step_complete', False) or len(st.session_state['e6_batch_list']) > 0:
            if c2.button("Next: Review Data", type="primary", key="next_e6"): st.session_state['entry_stage'] = 'env_review'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_e6"): st.session_state['entry_stage'] = 'env_review'; st.rerun()
        ...
        def classify_entry(name):
            n = name.lower()
            if "electricity" in n or "grid mix" in n or "renewable" in n or "nuclear" in n: return "E1 Electricity", "Scope 2"
            elif "district heating" in n or "cooling" in n or "steam" in n: return "E1 Heating", "Scope 2"
            elif "scope 1" in n or "diesel" in n or "petrol" in n or "natural gas" in n: return "E2 Fleet/Heat", "Scope 1"
            elif "flight" in n or "train" in n or "hotel" in n or "private car" in n: return "E3 Travel", "Scope 3"
            elif "commute" in n: return "E4 Commuting", "Scope 3"
            elif "waste" in n: return "E5 Waste", "Scope 3"
            elif "water" in n: return "E3 Water", "Scope 3"
            return "Other", "Scope 3"

        def get_type_from_string(name):
            if "estimate" in name.lower() or "calc" in name.lower(): return "Estimated"
            return "Measured"
        ...
        if db_rows:
            for row in db_rows:
                ...
                all_data.append({'Module': mod, 'Scope': scope, 'CO2': row['co2_kg'], 'Data Type': dtype})

        if all_data:
            df_review = pd.DataFrame(all_data)
            total_co2 = df_review['CO2'].sum()
            c1, c2, c3 = st.columns(3)
            with c1: st.container(border=True).metric("Total Footprint", f"{total_co2/1000:.2f} t", "CO2e")
            with c2:
                if not df_review.empty:
                    top_driver = df_review.groupby("Module")['CO2'].sum().idxmax()
                    share = df_review.groupby("Module")['CO2'].sum().max() / total_co2 * 100
                else:
                    top_driver = "N/A"; share = 0
            ...
            with c3:
                ...
                st.container(border=True).metric("Data Accuracy", f"{100-est_share:.1f}%")
            st.markdown("### Composition by Category")
            with st.container(border=True):
                base = alt.Chart(df_review).encode(theta=alt.Theta("CO2", stack=True))
                pie = base.mark_arc(outerRadius=120, innerRadius=80).encode(color=alt.Color("Module"), order=alt.Order("CO2", sort="descending"), tooltip=["Module", "CO2", "Scope"])
                text = base.mark_text(radius=140).encode(text=alt.Text("CO2", format=".0f"), order=alt.Order("CO2", sort="descending"), color=alt.value("black"))
            ...
            with c_left:
                with st.container(border=True):
                    ...
                    c = alt.Chart(chart_data).mark_bar(color='#023425').encode(x='Scope', y='CO2')
                    st.altair_chart(c, use_container_width=True)
            with c_right:
                with st.container(border=True):
                    st.markdown("**Data Quality (Measured vs. Estimated)**")
                    c = alt.Chart(df_review).mark_bar().encode(x='Module', y='CO2', color=alt.Color('Data Type', scale=alt.Scale(domain=['Measured', 'Estimated'], range=['#023425', '#86efac'])))
                    st.altair_chart(c, use_container_width=True)
            with st.expander("View Qualitative Policy Check"):
                policy_rows = [r for r in db_rows if r['co2_kg'] == 0]
                if policy_rows:
                    for p in policy_rows: st.write(f"- {p['fuel_type']}")
                else:
                    st.info("No strategies uploaded yet.")
        else:
            st.info("No data found in Database.")
        st.markdown("---")
        if st.button(t("complete_return", lang), type="primary", use_container_width=True):
            st.session_state['entry_stage'] = 'main'
            st.rerun()
    ...
    # --- SOCIAL PILLAR ---
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'soc_intro'; st.rerun()
        ...
        with st.container(border=True):
            ...
            with c2:
                if hc > 0: st.metric("Female Share (Calculated)", f"{(women / hc) * 100:.1f}%")
                else: st.metric("Female Share", "0.0%")
            st.markdown("---")
            st.markdown("**Age Structure (Number of Employees)**")
            c1, c2, c3 = st.columns(3)
            with c1: u30 = st.number_input("Under 30 years", min_value=0, max_value=hc, key="widget_s1_age_u30")
            with c2:
                remaining = hc - u30
                mid = st.number_input("30 - 50 years", min_value=0, max_value=max(0, remaining), key="widget_s1_age_mid")
            with c3: st.metric("Over 50 years (Auto)", f"{hc - u30 - mid}")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_s1_basic_direct, type="primary", use_container_width=True)
            if st.session_state.get('s1_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s1_final"): st.session_state['entry_stage'] = 's1_intro'; st.rerun()
        if st.session_state.get('s1_step_complete', False):
            if c2.button("Next: S1.2 Health & Safety", type="primary", key="next_s1_final"): st.session_state['entry_stage'] = 's1_2_form'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s1_final"): st.session_state['entry_stage'] = 's1_2_form'; st.rerun()
        ...
        with st.container(border=True):
            c1, c2 = st.columns(2)
            with c1:
                st.number_input("Number of Work Accidents", min_value=0, key="widget_s1_accidents")
                st.number_input("Fatalities (Work-related)", min_value=0, key="widget_s1_fatalities")
            with c2:
                st.number_input("Sickness Rate (%)", min_value=0.0, max_value=100.0, step=0.1, key="widget_s1_sick")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_s1_health_direct, type="primary", use_container_width=True)
            if st.session_state.get('s1_2_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s1_2"): st.session_state['entry_stage'] = 's1_form'; st.rerun()
        if st.session_state.get('s1_2_step_complete', False):
            if c2.button("Next: S1.3 Wages", type="primary", key="fin_s1_2"): st.session_state['entry_stage'] = 's1_3_form'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s1_2"): st.session_state['entry_stage'] = 's1_3_form'; st.rerun()

    elif st.session_state['entry_stage'] == 's1_3_form':
        st.subheader("S1.3 Wages & Equality")
        st.info("**Data Privacy Note:** Salary data is sensitive. Your inputs are stored securely and only used to calculate the required ESG ratios.")
        with st.container(border=True):
            st.markdown("**1. Gender Pay Gap (Average Gross Yearly Salary)**")
            c1, c2 = st.columns(2)
            with c1: men = st.number_input("Ø Salary Men (€)", min_value=0.0, step=1000.0, key="widget_s1_sal_men")
            with c2: women = st.number_input("Ø Salary Women (€)", min_value=0.0, step=1000.0, key="widget_s1_sal_women")
            if men > 0 and women > 0:
                gap = ((men - women) / men) * 100
                color = "normal" if gap < 5 else "off"
                st.metric("Calculated Pay Gap", f"{gap:.1f}%", delta_color=color)
            st.markdown("---")
            st.markdown("**2. Fair Remuneration**")
            c1, c2 = st.columns(2)
            with c1: st.selectbox("Top Management vs. Average Worker Pay", ["Fair / Standard (< 1:10)", "High Disparity (> 1:10)", "Unknown / Confidential"], key="widget_s1_ceo")
            with c2: st.selectbox("Do you pay a 'Living Wage'?", ["Yes, all above minimum standard", "No / Unsure", "Partially"], key="widget_s1_living_wage")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_s1_wages_direct, type="primary", use_container_width=True)
            if st.session_state.get('s1_3_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s1_3"): st.session_state['entry_stage'] = 's1_2_form'; st.rerun()
        if st.session_state.get('s1_3_step_complete', False):
            if c2.button("Next: S1.4 Training", type="primary", key="fin_s1_3"): st.session_state['entry_stage'] = 's1_4_form'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s1_3"): st.session_state['entry_stage'] = 's1_4_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            c1, c2 = st.columns(2)
            with c1: total_h = st.number_input("Total Hours (All Staff)", min_value=0, step=10, key="widget_s1_hours_total")
            with c2:
                if last_total > 0: st.metric("Average per Employee", f"{(total_h/last_total):.1f} h")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_s1_training_direct, type="primary", use_container_width=True)
            if st.session_state.get('s1_4_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s1_4"): st.session_state['entry_stage'] = 's1_3_form'; st.rerun()
        if st.session_state.get('s1_4_step_complete', False):
            if c2.button("Next: S2 Value Chain", type="primary", key="fin_s1_4"): st.session_state['entry_stage'] = 's2_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s1_4"): st.session_state['entry_stage'] = 's2_intro'; st.rerun()
        ...
        with st.container(border=True):
            ...
            **Focus: The people behind your value chain.**
            ...
            <strong>Note:</strong> This applies to <u>ALL suppliers</u>, including <strong>Software providers (SaaS)</strong>, <strong>Office Supplies</strong>, <strong>Cleaners</strong>, and <strong>Consultants</strong>.
            </div>
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s2_intro"): st.session_state['entry_stage'] = 's1_4_form'; st.rerun()
        if c2.button("Next: Module S2 (Form)", type="primary", key="start_s2"): st.session_state['entry_stage'] = 's2_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            st.selectbox("Status of Code of Conduct", ["No Code of Conduct", "Internal Guidelines exist (not shared)", "Yes, sent to suppliers", "Yes, signed bindingly by key suppliers"], key="widget_s2_coc")
            st.markdown("---")
            st.markdown("**2. Risk Analysis**")
            st.selectbox("Risk Management Approach", ["No formal analysis", "Ad-hoc checks (Google/News)", "Systematic screening of key suppliers", "Full supply chain mapping (Tier 1 & Tier 2)"], key="widget_s2_risk")
            st.markdown("---")
            st.markdown("**3. Audits & Checks**")
            st.selectbox("Audit Mechanism", ["None", "Self-Assessment Questionnaires (Suppliers fill out form)", "On-site Visits by our team", "Third-party Audits (e.g. SMETA, SA8000)"], key="widget_s2_audit")
            st.button(t("add_to_list", lang), on_click=add_s2_to_batch, type="primary")
        st.markdown("---")
        if st.session_state['s2_batch_list']:
            ...
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_s2, key="rm_s2")
            c2.button(t("confirm_upload", lang), on_click=upload_s2_batch, key="up_s2")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s2"): st.session_state['entry_stage'] = 's2_intro'; st.rerun()
        has_data = st.session_state['s2_step_complete'] or len(st.session_state['s2_batch_list']) > 0
        if has_data:
            if c2.button("Next: S3 Communities", type="primary", key="fin_s2"): st.session_state['entry_stage'] = 's3_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s2"): st.session_state['entry_stage'] = 's3_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s3_intro"): st.session_state['entry_stage'] = 's2_form'; st.rerun()
        if c2.button("Next: Module S3 (Form)", type="primary", key="start_s3"): st.session_state['entry_stage'] = 's3_form'; st.rerun()

    elif st.session_state['entry_stage'] == 's3_form':
        st.subheader("S3 Community Impact")
        with st.container(border=True):
            st.markdown("**1. Operational Impact (Negative)**")
            st.selectbox("Impact Level", ["None (Office / Remote Work)", "Low (Some delivery traffic)", "Medium (Regular noise / Shift work)", "High (Industrial noise / Emissions)"], key="widget_s3_impact")
            st.markdown("---")
            st.markdown("**2. Social Engagement (Positive)**")
            st.selectbox("Type of Engagement", ["None", "Occasional Donations (Christmas etc.)", "Regular Sponsorship (Sports/Culture)", "Corporate Volunteering (Time off for social work)"], key="widget_s3_engage")
            st.text_input("Optional: Description of activities", placeholder="e.g. Donated 5 laptops to local school...", key="widget_s3_desc")
            st.button(t("add_to_list", lang), on_click=add_s3_to_batch, type="primary")
        st.markdown("---")
        if st.session_state['s3_batch_list']:
            ...
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_s3, key="rm_s3")
            c2.button(t("confirm_upload", lang), on_click=upload_s3_batch, key="up_s3")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s3_fix"): st.session_state['entry_stage'] = 's3_intro'; st.rerun()
        has_data = st.session_state['s3_step_complete'] or len(st.session_state['s3_batch_list']) > 0
        if has_data:
            if c2.button("Next: S4 Consumers", type="primary", key="fin_s3_fix"): st.session_state['entry_stage'] = 's4_intro'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s3_fix"): st.session_state['entry_stage'] = 's4_intro'; st.rerun()
        ...
        st.divider()
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s4_intro"): st.session_state['entry_stage'] = 's3_form'; st.rerun()
        if c2.button("Next: Module S4 (Form)", type="primary", key="start_s4"): st.session_state['entry_stage'] = 's4_form'; st.rerun()

    elif st.session_state['entry_stage'] == 's4_form':
        st.subheader("S4 Consumer Protection")
        with st.container(border=True):
            st.markdown("**1. Data Privacy (GDPR / DSGVO)**")
            st.selectbox("Privacy Standard", ["Basic Compliance (Privacy Policy on Website)", "Advanced (Cookie Consent Manager & regular deletion)", "High Security (ISO 27001 certified)", "Not Applicable (B2B / No personal data)"], key="widget_s4_privacy")
            st.markdown("---")
            st.markdown("**2. Product Safety & Quality**")
            st.selectbox("Safety Measures", ["Legal Standard (CE Marking / Local Law)", "Quality Management System (ISO 9001)", "Extended Warranty & Support", "Not Applicable (Service only)"], key="widget_s4_safety")
            st.markdown("---")
            st.markdown("**3. Responsible Marketing**")
            st.selectbox("Marketing Policy", ["No formal policy", "Internal Code of Ethics (Honesty, Fairness)", "Strict Anti-Greenwashing Guidelines", "Protection of Minors / Vulnerable Groups"], key="widget_s4_marketing")
            st.button(t("add_to_list", lang), on_click=add_s4_to_batch, type="primary")
        st.markdown("---")
        if st.session_state['s4_batch_list']:
            ...
            c1, c2 = st.columns(2)
            c1.button(t("remove_last", lang), on_click=remove_last_s4, key="rm_s4")
            c2.button(t("confirm_upload", lang), on_click=upload_s4_batch, key="up_s4")
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_s4"): st.session_state['entry_stage'] = 's4_intro'; st.rerun()
        has_data = st.session_state['s4_step_complete'] or len(st.session_state['s4_batch_list']) > 0
        if has_data:
            if c2.button("Next: Review Data", type="primary", key="fin_s4"): st.session_state['entry_stage'] = 'soc_review'; st.rerun()
        else:
            if c2.button(t("skip", lang), key="skip_s4"): st.session_state['entry_stage'] = 'soc_review'; st.rerun()
        ...
        try:
            ...
            rows = []
        if rows:
            social_rows = [r for r in rows if any(k in str(r.get('fuel_type', '')) for k in ["S1:", "S1.", "S2:", "S3:", "S4:"])]
            if social_rows:
                ...
                if s1_data:
                    ...
                    for item in s1_data:
                        ...
                        clean_s1.append({"Indicator": display_name, "Value": item.get('value_raw', 0), "Details": raw_name})
                ...
                if other_data:
                    ...
                    for item in other_data:
                        ...
                        with st.expander(name.split('(')[0].strip()):
                            st.write(name)
                else:
    ...
    # --- GOVERNANCE PILLAR ---
        ...
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang)): st.session_state['entry_stage'] = 'main'; st.rerun()
        if c2.button("Next: Module G1 (Ethics)", type="primary"): st.session_state['entry_stage'] = 'g1_form'; st.rerun()
        ...
        with st.container(border=True):
            ...
            st.selectbox("Policy Status", ["No formal policy", "Internal guidelines (verbal/written)", "Strict Anti-Corruption Policy signed by all staff"], key="widget_g1_corruption")
            st.markdown("---")
            st.markdown("**2. Whistleblower Protection**")
            st.selectbox("Reporting Channel", ["None", "Direct supervisor only", "Anonymous Mailbox / Digital Channel (EU Compliant)"], key="widget_g1_whistle")
            st.markdown("---")
            st.markdown("**3. Political Influence (Lobbying)**")
            st.selectbox("Political Contributions", ["No political contributions", "Transparent donations (Publicly disclosed)", "Undisclosed lobbying activities"], key="widget_g1_lobby")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_g1_direct, type="primary", use_container_width=True)
            if st.session_state.get('g1_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_g1"): st.session_state['entry_stage'] = 'gov_intro'; st.rerun()
        if st.session_state.get('g1_step_complete', False):
            if c2.button("Next: G2 Management", type="primary", key="next_g1"): st.session_state['entry_stage'] = 'g2_form'; st.rerun()
        else:
        ...
        with st.container(border=True):
            ...
            st.selectbox("Accountability", ["No dedicated person", "Shared among regular staff", "Dedicated ESG Officer / Manager", "Executive Board / CEO"], key="widget_g2_resp")
            st.markdown("---")
            st.markdown("**2. Risk Management**")
            st.selectbox("Risk Integration", ["No, not tracked", "Tracked informally", "Yes, fully integrated into company risk management"], key="widget_g2_risk")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_g2_direct, type="primary", use_container_width=True)
            if st.session_state.get('g2_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_g2"): st.session_state['entry_stage'] = 'g1_form'; st.rerun()
        if st.session_state.get('g2_step_complete', False):
            if c2.button("Next: G3 Suppliers", type="primary", key="next_g2"): st.session_state['entry_stage'] = 'g3_form'; st.rerun()
        else:
        ...
        with st.container(border=True):
            ...
            st.selectbox("Standard Payment Terms", ["0-30 days", "31-60 days", "61-90 days", "More than 90 days"], key="widget_g3_pay")
            st.markdown("---")
            st.markdown("**2. Local Sourcing**")
            st.selectbox("Local Supplier Spend", ["Less than 10%", "10% - 25%", "26% - 50%", "Over 50%"], key="widget_g3_local")
            st.markdown("---")
            st.button(t("confirm_upload", lang), on_click=save_g3_direct, type="primary", use_container_width=True)
            if st.session_state.get('g3_step_complete'): st.success(t("saved_successfully", lang))
        st.markdown("---")
        c1, c2 = st.columns([1, 4])
        if c1.button(t("back", lang), key="back_g3"): st.session_state['entry_stage'] = 'g2_form'; st.rerun()
        if st.session_state.get('g3_step_complete', False):
            if c2.button("Next: Review Data", type="primary", key="finish_g3"): st.session_state['entry_stage'] = 'gov_overview'; st.rerun()
        else:
        ...
        with c1:
            with st.container(border=True):
                ...
                st.write(f"**{st.session_state.get('widget_g1_corruption', 'Not provided')}**")
        with c2:
            with st.container(border=True):
                st.markdown("### G2: Management")
                st.caption("ESG Responsibility:")
                st.write(f"**{st.session_state.get('widget_g2_resp', 'Not provided')}**")
                st.caption("Risk Integration:")
                st.write(f"**{st.session_state.get('widget_g2_risk', 'Not provided')}**")
        with c3:
            with st.container(border=True):
                st.markdown("### G3: Suppliers")
                st.caption("Payment Terms:")
                st.write(f"**{st.session_state.get('widget_g3_pay', 'Not provided')}**")
                st.caption("Local Sourcing:")
                st.write(f"**{st.session_state.get('widget_g3_local', 'Not provided')}**")
        st.markdown("---")
        if st.button(t("complete_return", lang), type="primary", use_container_width=True):
    ...
    if raw_data:
        if st.button(t("generate_report", lang), type="primary", use_container_width=True):
            ...
            try:
                ...
                prompt_intro = f"{t('ai_prompt_intro', lang)} You are a senior ESG auditor. Write the 'Introduction & Business Model' chapter. Use the provided Company Description and Industry to explain their value chain. Add a sub-chapter about their 'Double Materiality Assessment'. Be highly formal, analytical, and objective. No emojis. Format the main heading as '## 0. Business Model & Materiality'."
                completion_intro = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": prompt_intro}, {"role": "user", "content": user_data}], temperature=0.2)
                full_report += completion_intro.choices[0].message.content + "\n\n"

                my_bar.progress(10, text="Analyzing Environmental Data (ESRS E1-E5)...")
                prompt_env = f"{t('ai_prompt_env', lang)} You are a senior ESG auditor. Focus ONLY on environmental metrics (CO2 emissions, energy, waste, water). Cite the exact ESRS tags. Incorporate the specific context and notes provided in the 'description' field. Be highly formal, analytical, and objective. No emojis."
                completion_env = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": prompt_env}, {"role": "user", "content": user_data}], temperature=0.2)
                ...
                prompt_soc = f"{t('ai_prompt_soc', lang)} You are a senior ESG auditor. Focus ONLY on social metrics (workforce, health & safety, gender pay gap, training). Cite the exact ESRS tags. Be highly formal, analytical, and objective. No emojis."
                completion_soc = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": prompt_soc}, {"role": "user", "content": user_data}], temperature=0.2)
                ...
                prompt_gov = f"{t('ai_prompt_gov', lang)} You are a senior ESG auditor. Focus ONLY on governance metrics (policies, risk management, anti-corruption, supplier relations). Cite the exact ESRS tags. Be highly formal, analytical, and objective. No emojis."
                completion_gov = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": prompt_gov}, {"role": "user", "content": user_data}], temperature=0.2)
                ...
                st.success(t("report_generated", lang))
                st.markdown("---")
                st.markdown(f"### {t('review_edit', lang)}")
                st.markdown(t("review_edit_desc", lang))
                ...
                fig1 = go.Figure(go.Indicator(mode="number+gauge", value=quality_score, number={'font': {'size': 28, 'color': '#333333', 'family': 'Arial'}, 'suffix': "%"}, title={'text': "Data<br>Quality", 'font': {'size': 13, 'color': '#777777', 'family': 'Arial'}}, gauge={'shape': "bullet", 'axis': {'range': [0, 100], 'tickvals': [0, 50, 100], 'ticktext': ['0%', '50%', '100%'], 'showticklabels': True, 'tickfont': dict(color='#777777', size=11)}, 'bar': {'color': "#023425", 'thickness': 0.35}, 'bgcolor': "rgba(0,0,0,0)", 'borderwidth': 0}))
                fig1.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=80, r=20, t=50, b=40), height=180)

                risk_steps = []
                for i in range(50):
                    if i < 25:
                        r = int(134 + (253 - 134) * (i / 25)); g = int(239 + (224 - 239) * (i / 25)); b = int(172 + (71 - 172) * (i / 25))
                    else:
                        r = int(253 + (248 - 253) * ((i - 25) / 25)); g = int(224 + (113 - 224) * ((i - 25) / 25)); b = int(71 + (113 - 71) * ((i - 25) / 25))
                    risk_steps.append({'range': [i, i+1], 'color': f'rgb({r},{g},{b})'})

                fig_risk = go.Figure(go.Indicator(mode="number+gauge", value=user_risk, number={'font': {'size': 28, 'color': '#333333', 'family': 'Arial'}}, title={'text': "ESG Risk<br>Score", 'font': {'size': 13, 'color': '#777777', 'family': 'Arial'}}, gauge={'shape': "bullet", 'axis': {'range': [0, 50], 'tickvals': [0, 25, 50], 'ticktext': ['Low', 'Med', 'Severe'], 'tickfont': dict(color='#777777', size=11)}, 'bar': {'color': "#333333", 'thickness': 0.15}, 'bgcolor': "rgba(0,0,0,0)", 'borderwidth': 0, 'steps': risk_steps, 'threshold': {'line': {'color': "#333333", 'width': 2}, 'thickness': 0.8, 'value': base_risk}}))
                fig_risk.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=80, r=20, t=50, b=40), height=180)

                fig2 = go.Figure()
                fig2.add_trace(go.Bar(y=['Scope 3', 'Scope 2', 'Scope 1'], x=[s3_co2, s2_co2, s1_co2], orientation='h', marker_color=['#86efac', '#416852', '#023425'], width=0.4))
                fig2.update_layout(title={'text': "Carbon Footprint (t CO2e)", 'font': {'size': 14, 'color': '#777777', 'family': 'Arial'}}, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=70, r=20, t=40, b=30), height=360, showlegend=False, xaxis=dict(showgrid=True, gridcolor='#eeeeee', zeroline=False, tickfont=dict(color='#777777', size=11)), yaxis=dict(showgrid=False, tickfont=dict(color='#555555', size=12)))
                ...
                try:
                    ...
                    gauge_path = "temp_gauge.png"; risk_path = "temp_risk.png"; bar_path = "temp_bar.png"
                except Exception as e:
                    gauge_path = None; risk_path = None; bar_path = None
                ...
                pdf_bytes = generate_audit_pdf(company, year, edited_report, df_for_pdf, country=hq, gauge_img=gauge_path, bar_img=bar_path, risk_img=risk_path, star_rating=calculated_star_rating)

                st.download_button(label=t("download_report", lang), data=pdf_bytes, file_name=f"CSRD_Audit_Report_{company}_{year}.pdf", mime="application/pdf", type="primary", use_container_width=True)

            except Exception as e:
                st.error(f"{t('ai_failed', lang)} {e}")
                my_bar.empty()
    else:
        st.info(t("no_data_report", lang))
    ...
    tab_locations, tab_upload, tab_overview = st.tabs(["Manage Locations", "Monthly Upload", "Overview & History"])
    ...
    with tab_locations:
        ...
        with st.expander("Add New Location", expanded=(len(existing_locations) == 0)):
            with st.form("new_location_form"):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    new_loc_name = st.text_input("Location Name *", placeholder="e.g. Vienna Headquarters, Warehouse Graz...")
                    new_loc_type = st.selectbox("Location Type", LOCATION_TYPES)
                with c2:
                    new_loc_address = st.text_input("Address (optional)", placeholder="e.g. Mariahilfer Str. 1, 1060 Vienna")
                    new_loc_country = st.selectbox("Country", ["Austria", "Germany", "Switzerland", "France", "Italy", "Spain", "Netherlands", "Belgium", "Sweden", "Denmark", "Poland", "Ireland", "Singapore", "Philippines", "United Kingdom", "USA", "Other"])
                ...
                if submitted:
                    if not new_loc_name.strip():
                        ...
                        try:
                            supabase.table("esg_locations").insert({"company": company_name, "location_name": new_loc_name.strip(), "location_type": new_loc_type, "address": new_loc_address.strip(), "country": new_loc_country}).execute()
            ...
            for loc in existing_locations:
                ...
                with col_info:
                    with st.container(border=True):
                        ...
                        if loc.get('address'): st.caption(f"Address: {loc['address']}")
                with col_del:
                    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with tab_upload:
        ...
        if not locations_for_upload:
            ...
            with st.container(border=True):
                ...
                with c1: selected_loc_name = st.selectbox("Location", options=list(loc_options.keys()), key="portal_loc_select")
                with c2: selected_year = st.selectbox("Year", ["2023", "2024", "2025", "2026"], index=1, key="portal_year_select")
                with c3: selected_month = st.selectbox("Month", MONTHS, key="portal_month_select")

            selected_loc = loc_options.get(selected_loc_name)
            already_uploaded = []
            try:
                existing_resp = supabase.table("esg_monthly_uploads").select("category").eq("company", company_name).eq("location_id", selected_loc['id']).eq("year", selected_year).eq("month", selected_month).execute()
            ...
            with st.form("monthly_upload_form"):
                for cat_name, cat_info in MONTHLY_CATEGORIES.items():
                    ...
                    with st.container(border=True):
                        ...
                        with col_val:
                            val = st.number_input(f"Value ({cat_info['unit']})", min_value=0.0, step=1.0, key=f"val_{cat_name}", disabled=already_done, label_visibility="collapsed")
                            if val > 0: st.caption(f"approx. {val * cat_info['co2_factor']:.1f} kg CO2")
                        with col_file:
                            file_key = f"file_{cat_name}_{selected_loc['id']}_{selected_year}_{selected_month}"
                            st.file_uploader("Upload Bill", type=["pdf", "jpg", "png", "jpeg"], key=file_key, disabled=already_done, label_visibility="collapsed")
                        st.text_input("Note (optional)", placeholder="e.g. Invoice #123, Meter ID...", key=f"note_{cat_name}", disabled=already_done, label_visibility="collapsed")

                submit_all = st.form_submit_button(f"Save All Entries for {selected_month} {selected_year}", type="primary", use_container_width=True)
                if submit_all:
                    saved_count = 0; error_count = 0
                    for cat_name, cat_info in MONTHLY_CATEGORIES.items():
                        if cat_name in already_uploaded: continue
                        ...
                        try:
                            supabase.table("esg_monthly_uploads").insert({"company": company_name, "location_id": selected_loc['id'], "location_name": selected_loc_name, "year": selected_year, "month": selected_month, "category": cat_name, "value_raw": val, "unit": cat_info['unit'], "co2_kg": co2, "esrs_tag": cat_info['esrs_tag'], "notes": f"{note} | Scope: {cat_info['scope']}", "file_url": file_url}).execute()
                            saved_count += 1
                        except Exception as e:
                            st.error(f"Failed to save {cat_name}: {e}"); error_count += 1
                    if saved_count > 0:
    ...
    with tab_overview:
        ...
        all_uploads = []
        try:
            overview_resp = supabase.table("esg_monthly_uploads").select("*").eq("company", company_name).order("year", desc=True).execute()
        ...
        if not all_uploads:
            ...
            with st.container(border=True):
                ...
                with fc1: filter_loc = st.multiselect("Location", options=sorted(df_uploads['location_name'].unique()), default=sorted(df_uploads['location_name'].unique()))
                with fc2: filter_year = st.multiselect("Year", options=sorted(df_uploads['year'].unique(), reverse=True), default=sorted(df_uploads['year'].unique(), reverse=True))
                with fc3: filter_cat = st.multiselect("Category", options=sorted(df_uploads['category'].unique()), default=sorted(df_uploads['category'].unique()))
            df_filtered = df_uploads[df_uploads['location_name'].isin(filter_loc) & df_uploads['year'].isin(filter_year) & df_uploads['category'].isin(filter_cat)]
            if not df_filtered.empty:
                st.markdown("### CO2 by Location & Category")
                chart_data = df_filtered.groupby(['location_name', 'category'])['co2_kg'].sum().reset_index()
                chart = alt.Chart(chart_data).mark_bar().encode(x=alt.X('location_name:N', title='Location'), y=alt.Y('co2_kg:Q', title='CO2 (kg)'), color=alt.Color('category:N'), tooltip=['location_name', 'category', 'co2_kg']).properties(height=300)
            ...
            st.markdown("---")
            with st.expander(t("delete_record", lang)):
                if 'id' in df_filtered.columns:
                    del_options = {f"ID {r['id']} — {r['location_name']} | {r['month']} {r['year']} | {r['category']}": r['id'] for _, r in df_filtered.iterrows()}
                    selected_del = st.selectbox(t("select_record_delete", lang), list(del_options.keys()))
                    if st.button(t("delete_button", lang), type="primary"):
                        try:
    ...
    st.info(t("settings_coming_soon", lang))
    st.markdown(f"### {t('two_factor_auth', lang)}")
    if st.button(t("setup_authenticator", lang)):
            ...
            with st.form("confirm_mfa_form"):
                ...
                if st.form_submit_button("Confirm & Activate"):
                    if verify_mfa(confirm_code):
                        st.success(t("mfa_activated", lang))
